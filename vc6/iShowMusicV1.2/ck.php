...
require_once('inc/text_function.php');
...
if(!$sid){
	$sid = randstr(8);
	Cookie('sid',$sid);
	$ckline="$sid|$nmsg|$timestamp|\n";
    $ckdata="$datadir/cknum.php";
    text_modify($ckdata,$sid,"|","0",$ckline);
}else{
	$oldsid=$sid;
	$newsid = randstr(8);
	Cookie('sid',$newsid);
	$ckline="$newsid|$nmsg|$timestamp|\n";
    $ckdata="$datadir/cknum.php";
	text_delete($ckdata,$oldsid,"|","0");
	writeover($ckdata,$ckline,"a+");
}
